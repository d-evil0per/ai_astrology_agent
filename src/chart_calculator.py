...
ORBS = {
    ...
    "Saturn": {"Conjunction": 9, "Sextile": 5, "Square": 7, "Trine": 7, "Opposition": 9},
    "Uranus": {"Conjunction": 6, "Sextile": 3, "Square": 5, "Trine": 5, "Opposition": 6},
    "Neptune": {"Conjunction": 6, "Sextile": 3, "Square": 5, "Trine": 5, "Opposition": 6},
    ...
    "Ascendant": {"Conjunction": 5, "Sextile": 3, "Square": 5, "Trine": 5, "Opposition": 5},
    "Midheaven": {"Conjunction": 5, "Sextile": 3, "Square": 5, "Trine": 5, "Opposition": 5},
}

# Default orb for aspects between outer planets or to nodes/angles if not specified above
DEFAULT_ORB = {"Conjunction": 3, "Sextile": 2, "Square": 3, "Trine": 3, "Opposition": 3}
...
class ChartCalculator:
    def __init__(self, ephemeris_path=EPHEMERIS_PATH, geocode_api_key=GEOCODE_API_KEY):
        ...
        try:
            ...
            print(f"ERROR: Geocode API key file not found at {geocode_api_key_path}")
    ...
    def get_coordinates_and_timezone(self, city_country_str):
        ...
        try:
            ...
            if data and isinstance(data, list) and len(data) > 0:
                ...
                if not timezone_str:
                    # Fallback for locations in ocean or if timezonefinder fails
                    # This is a very rough fallback and might not be accurate
                    offset_hours = int(lon / 15)
                    timezone_str = f"Etc/GMT{-offset_hours:+d}" # Note: Etc/GMT signs are reversed
    ...
    def get_utc_datetime_and_julian_day(self, year, month, day, hour, minute, lat, lon, timezone_str):
        """Converts local time to UTC and calculates Julian Day."""
        try:
        ...
        # This handles DST automatically if applicable for that timezone and date
        try:
            localized_dt = local_tz.localize(local_dt, is_dst=None) # is_dst=None makes pytz raise an error for ambiguous or non-existent times
        except (pytz.exceptions.AmbiguousTimeError, pytz.exceptions.NonExistentTimeError) as e:
             # Attempt to resolve ambiguity, e.g., by picking the first occurrence or standard time
            try:
                localized_dt = local_tz.localize(local_dt, is_dst=False) # try standard time
        ...
        # Julian day for ephemeris calculation (UT)
        jd_ut = swe.utc_to_jd(utc_dt.year, utc_dt.month, utc_dt.day, 
                              utc_dt.hour, utc_dt.minute, utc_dt.second, 1)[1] # 1 for Gregorian calendar
...
if __name__ == "__main__":
    ...
    ny_chart = calculator.calculate_natal_chart(2000, 1, 1, 12, 0, "New York, NY, USA")
